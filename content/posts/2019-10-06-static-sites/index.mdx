...
One minor hickup with Gatsby was that I had to set `pathPrefix: "/blog"` in the `gatsby-config.js` to have every link and asset path realize that it's being served from a directory.
